...
HIGH_RISK_KEYWORDS = {
    "fraud", "stolen", "steal", "identity theft", "compromised",
    "phishing", "chargeback", "dispute", "urgent", "immediately",
    "security breach", "private info", "personal data", "expiration",
    "start time", "end time", "password", "access lost", "rejected me",
    "review my answers", "increase my score", "move me to the next round",
    "site is down", "pages are inaccessible", "none of the pages",
}

UNSUPPORTED_KEYWORDS = {
    "actor", "movie", "sports", "weather", "music", "news",
    "politics", "recipe", "joke", "quote", "random",
}
...
class SupportAgent:
        ...
        if best_article is None:
            if escalated:
                return TriageResult(
                    ...
                    response="",
                    justification=self._build_justification(company, request_type, best_article, True),
            ...
            return TriageResult(
                ...
                justification=self._build_justification(company, request_type, best_article, False),
                request_type=request_type,
            )
    ...
    def _resolve_company(self, ticket: Ticket) -> str | None:
        company = ticket.company.lower() if ticket.company and ticket.company.strip() and ticket.company.strip().lower() != "none" else None
        if company not in {"hackerrank", "claude", "visa"}:
            lowered = ticket.full_text.lower()
            if "hackerrank" in lowered: return "hackerrank"
            if "claude" in lowered: return "claude"
            if "visa" in lowered: return "visa"
            return None
        return company

    def _default_area(self, company: str | None) -> str:
        if company == "claude": return "conversation_management"
        if company == "hackerrank": return "general_help"
        if company == "visa": return "support"
        return "out_of_scope"

    def _classify_request_type(self, ticket: Ticket) -> str:
        text = ticket.full_text.lower()
        feature_markers = {
            "feature request", "can you add", "could you add", "would love",
            "i wish", "please add", "new feature", "enhancement", "improve",
        }
        if any(marker in text for marker in feature_markers):
            return "feature_request"
        if any(marker in text for marker in ["not working", "stopped", "broken", "error", "down", "failed", "bug", "glitch", "cannot", "can't", "won't"]):
            return "bug"
    ...
    def _looks_like_invalid(self, text: str) -> bool:
        ...
        if any(term in text for term in UNSUPPORTED_KEYWORDS): return True
        if len(tokenize(text)) < 3 and "hackerrank" not in text and "claude" not in text and "visa" not in text:
            return True
        return False

    def _should_escalate(self, ticket: Ticket, request_type: str, company: str | None, article: Article | None) -> bool:
        text = ticket.full_text.lower()
        if "site is down" in text or "pages are inaccessible" in text or "none of the pages" in text:
            return True
        
        if any(keyword in text for keyword in HIGH_RISK_KEYWORDS):
            article_title = article.title.lower() if article else ""
            if company == "visa" and any(term in text for term in ["stolen", "lost", "fraud", "dispute"]):
                return False
            if company == "claude" and (
                any(term in text for term in ["removed my seat", "workspace owner", "admin"])
                or any(term in article_title for term in ["incognito", "delete or rename", "conversation"])
            ):
                return False
            if company == "hackerrank" and "refund" in text:
                return False
            if company == "hackerrank" and article_title and any(term in article_title for term in ["modify test expiration time", "extend test duration"]):
                return False
            return True
    ...
    def _manual_topic_route(self, company: str | None, ticket: Ticket) -> tuple[str | None, list[str]]:
        text = ticket.full_text.lower()
        if company == "hackerrank":
            if "mock interview" in text:
                return "subscriptions_payments_and_billing", ["purchase mock interview credits", "subscriptions payments and billing faqs"]
            if any(term in text for term in ["extra time", "reinvite", "re-invite", "accommodation"]):
                return "screen", ["extend test duration for candidates"]
            if "variant" in text:
                return "screen", ["create test variants"]
            if any(term in text for term in ["test active", "stay active", "remain active", "how long", "expire", "expired", "invitation", "start time", "end time"]):
        ...
        if company == "claude":
            if any(term in text for term in ["private info", "delete", "conversation", "incognito"]):
                return "conversation_management", ["delete or rename a conversation", "using incognito chats"]
            if any(term in text for term in ["seat removed", "workspace owner", "admin", "access lost", "log in"]):
                return "account_management", ["how to get support", "logging in to your claude account"]
            if any(term in text for term in ["refund", "cancel subscription", "billing"]):
                return "plans", ["requesting a refund for a paid claude plan"]

        if company == "visa":
            if any(term in text for term in ["stolen", "lost card", "compromised", "fraud"]):
                return "consumer", ["visa travel services", "fraud prevention"]
            if any(term in text for term in ["traveller", "traveler", "cheque", "cheques"]):
                return "travelers_cheques", ["visa traveller's cheques"]
            if any(term in text for term in ["dispute", "refund me", "wrong product", "merchant", "charge"]):
                return "dispute_resolution", ["dispute resolution", "visa credit card rules"]
    ...
    def _build_justification(self, company: str | None, request_type: str, article: Article | None, escalated: bool) -> str:
        if escalated:
            if request_type == "invalid":
