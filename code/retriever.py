...
def derive_product_area(path: Path, company: str) -> str:
    parts = [part for part in path.with_suffix("").parts if part not in {"data", company}]
    if not parts:
    ...
    if company == "claude":
        ...
        return area_map.get(parts[1], parts[1].replace("-", "_")) if len(parts) > 1 else "claude"
    ...
    if company == "hackerrank":
        ...
        for part in parts[1:3]:
            if part in area_map:
                return area_map[part]
        return area_map.get(parts[1], parts[1].replace("-", "_")) if len(parts) > 1 else "hackerrank"
    ...
    if company == "visa":
        ...
        for part in parts[1:4]:
            if part in area_map:
                return area_map[part]
        return area_map.get(parts[1], parts[1].replace("-", "_")) if len(parts) > 1 else "visa"
...
def load_markdown_article(path: Path, data_root: Path) -> Article:
    ...
    company = path.parts[company_index] if len(path.parts) > company_index else "unknown"
...
class BM25Retriever:
    ...
    def _initialize(self):
        ...
        for article in self.corpus:
            doc_tokens = list(article.tokens) + list(article.heading_tokens) * 2 + list(tokenize(article.title)) * 3
    ...
    def retrieve(self, query: str, top_k: int = 5, company: str | None = None) -> list[Article]:
